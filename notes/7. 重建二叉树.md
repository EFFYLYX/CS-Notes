...
<div align="center"> <img src="https://github.com/EFFYLYX/CS-Notes/blob/master/notes/pics/IMG_C6D198BF08AA-1.jpeg" width="400"/> </div><br>
...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> TreeNode:
        ...
        def helper(preL, preR, inL):
            if preL > preR:
                return None
            
            root = TreeNode(preorder[preL])

            pivot_in_idx = idx_in[root.val]

            left_size = pivot_in_idx - inL
            
            root.left = helper( preL=preL+1, preR = preL+left_size, inL=inL)
            # root.right = helper( preL=preL+left_size+1, preR = preR, inL = inL+left_size+1)
            root.right = helper( preL=preL+left_size+1, preR = preR, inL = pivot_in_idx+1)

            return root
        return helper(preL=0,preR=len(preorder)-1,inL=0)
```
